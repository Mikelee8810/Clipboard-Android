...
import { clipboardManager } from './internal/clipboardManager';
import { clipboardMonitor } from './internal/clipboardMonitor';
...
export const useClipboardStore = create<ClipboardState>((set, get) => ({
  ...
  setContent: async (content: ClipboardContent) => {
    ...
    try {
      ...
      log.info(
        `new content set: type=${content.type}, text=${content.text?.substring(
          0,
          20
