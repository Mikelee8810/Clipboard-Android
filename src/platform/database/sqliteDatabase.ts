...
async function resolveIOSAppGroupDirectory(): Promise<string | null> {
  ...
  try {
    ...
    log.warn('App Group container unavailable, using sandbox directory:', error);
    return null;
  }
}
...
function migrateSandboxDatabaseFiles(targetDir: Directory): void {
  ...
  log.info('moved sandbox database into App Group container');
}
...
async function migrate(db: SQLite.SQLiteDatabase): Promise<void> {
  ...
  if (version === 0) {
    ...
    log.info('applied schema v1 (create table + indexes)');
    version = 1;
  }

  if (version === 1) {
    await migrateToV2(db);
    log.info('applied schema v2 (activate_clipboard + clipboard_history.contentId)');
    version = 2;
  }

  if (version === 2) {
    await migrateToV3(db);
    log.info('applied schema v3 (P2P delivery state)');
    version = 3;
  }

  if (version === 3) {
    await migrateToV4(db);
    log.info('applied schema v4 (P2P delivery counts)');
    version = 4;
  }
