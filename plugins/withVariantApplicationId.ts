 ...
 * automatically).
 */
const withVariantApplicationId: ConfigPlugin<VariantApplicationIdProps> = (config, { suffix }) => {
  if (!suffix) {
  ...
  return withAppBuildGradle(config, (config) => {
    ...
    const applicationIdRe = new RegExp(
      `applicationId (['"])${BASE_APPLICATION_ID.replace(/\./g, '\\.')}\\1`
    );
    ...
    if (!applicationIdRe.test(contents)) {
      if (
        ...
        console.warn(
          `⚠ withVariantApplicationId: could not find "applicationId '${BASE_APPLICATION_ID}'" in build.gradle; applicationId left unchanged`
        );
      }
      return config;
    }
