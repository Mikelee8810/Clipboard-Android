...
export async function waitForConnectionTimeout({ platform, id, output }) {
  ...
  while (Date.now() < deadline) {
    const logs = platform === 'ios'
      ? await command('xcrun', ['simctl', 'spawn', id, 'log', 'show', '--last', '2m', '--style', 'compact', '--predicate', 'subsystem == "app.clipboard" AND category == "engine" AND eventMessage CONTAINS "timed_out"'], { timeout: 15_000 })
